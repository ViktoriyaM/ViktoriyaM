...
{    
    ...
    return inputValidation(configuredScales, inputLine);
}
/**
 ...
 * вызывает функцию проверки наличия всех файлов из XML-документа в указанном каталоге {@link  FilesManager#initializeDirectory(com.privateProject.Configuration) },
 * вызывает функцию получения списка объектов из сформированной конфигурации {@link Algorithm#updateObjects(com.privateProject.Configuration) },
 * проверяет есть ли файлы для чения и вызывает алгоритм для обработки файлов карты {@link Algorithm#reader(java.util.function.Supplier) }.
 *
...
public boolean controlAlgorithm(final String selectedScale)
{
    ...
    while (filesManager.hasNext())
    {
        try
        {
            status = algorithm.reader(filesManager::next);
            if (!status)
            {
