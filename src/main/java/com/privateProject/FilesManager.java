...
boolean initializeDirectory(Configuration configuration)
{
    ...
    try
    {
        ...
        try
        {
            Path pathReader = Paths.get(new File(filesPath).toURI());
...
GenerateNext()
{
    ...
    {
        LOGGER.error("Error: iterator of files names are not returned value " + ex);
    }
    ...
    {
        LOGGER.error("Error: unsupported encoding in file " + ex);
    }
